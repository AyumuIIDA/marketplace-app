...
import { CreateHumanSignatureUseCase, } from "./create-human-signature.usecase.js";
...
describe("CreateHumanSignatureUseCase", () => {
    ...
    it("should allow multiple World ID verification records for the same user and action", async () => {
        ...
        const first = await useCase.execute(createInput({ actionType: "LISTING_UPDATE", resourceId: "listing-1", payloadHash: "sha256:payload-1" }));
        const second = await useCase.execute(createInput({ actionType: "LISTING_UPDATE", resourceId: "listing-1", payloadHash: "sha256:payload-2" }));
        expect(first.verificationCount).toBe(1);
        expect(second.verificationCount).toBe(2);
        expect(transaction.worldIdVerificationRepository.verifications.map((verification) => verification.snapshot.nullifierHash)).toEqual(["same-nullifier", "same-nullifier"]);
    });
    it("should reject duplicate valid signatures for the same resource payload", async () => {
        const fixedNow = new Date("2026-06-09T00:00:00.000Z");
        const transaction = new FakeHumanSignatureTransaction();
        transaction.humanSignatureRepository.signatures.push(HumanSignature.createValid({
    ...
    it("should not open a transaction when World ID verification fails", async () => {
        ...
        const useCase = createUseCase({
            transaction,
            worldIdVerifier: new FakeWorldIdVerifier(undefined, new AppError("WORLD_ID_VERIFICATION_FAILED", "World ID verification failed.", 400)),
            fixedNow,
            ids: ["verification-1", "signature-1"],
        });
...
class FakeHumanSignatureTransaction {
