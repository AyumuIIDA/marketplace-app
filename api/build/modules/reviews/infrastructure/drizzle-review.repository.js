...
export class DrizzleReviewRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    ...
    async findSubmittedByOrderReviewer(orderId, reviewerId) {
        const [row] = await this.db
            .select()
            .from(reviews)
            .where(and(eq(reviews.orderId, orderId), eq(reviews.reviewerId, reviewerId), eq(reviews.status, "SUBMITTED")))
            .limit(1);
        if (row === undefined) {
            return undefined;
        }
